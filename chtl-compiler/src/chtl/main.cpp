#include "CHTLCompilerV2.h"
#include "ast/CHTLASTNodes.h"
#include <iostream>
#include <fstream>
#include <string>
#include <cstring>

void PrintUsage(const char* program) {
    std::cout << "CHTL Compiler v2.0 - Complete Implementation\n";
    std::cout << "Usage: " << program << " [options] <input.chtl> [output.html]\n";
    std::cout << "\nOptions:\n";
    std::cout << "  -h, --help           Show this help message\n";
    std::cout << "  -d, --debug          Enable debug mode\n";
    std::cout << "  -m, --minify         Disable pretty printing\n";
    std::cout << "  -f, --fragment       Generate HTML fragment only\n";
    std::cout << "  -o, --output <file>  Specify output file\n";
    std::cout << "  --ast                Print AST structure\n";
    std::cout << "\nExamples:\n";
    std::cout << "  " << program << " input.chtl\n";
    std::cout << "  " << program << " -d input.chtl output.html\n";
    std::cout << "  " << program << " --fragment input.chtl -o output.html\n";
}

// AST 打印器
class ASTPrinter : public chtl::ast::ASTVisitor {
public:
    ASTPrinter() : m_Indent(0) {}
    
    void Print(std::shared_ptr<chtl::ast::ASTNode> root) {
        root->Accept(this);
    }
    
    void VisitDocument(chtl::ast::DocumentNode* node) override {
        PrintNode("Document");
        m_Indent++;
        for (const auto& child : node->GetChildren()) {
            child->Accept(this);
        }
        m_Indent--;
    }
    
    void VisitElement(chtl::ast::ElementNode* node) override {
        PrintNode("Element: " + node->GetTagName());
        
        // 打印属性
        auto& attrs = node->GetAttributes();
        if (!attrs.empty()) {
            m_Indent++;
            for (const auto& attr : attrs) {
                PrintNode("@" + attr.first + " = \"" + attr.second + "\"");
            }
            m_Indent--;
        }
        
        m_Indent++;
        for (const auto& child : node->GetChildren()) {
            child->Accept(this);
        }
        m_Indent--;
    }
    
    void VisitText(chtl::ast::TextNode* node) override {
        PrintNode("Text: \"" + node->GetContent() + "\"");
    }
    
    void VisitStyle(chtl::ast::StyleNode* node) override {
        std::string type = (node->GetType() == chtl::ast::StyleNode::LOCAL) ? "local" : "global";
        PrintNode("Style (" + type + ")");
    }
    
    void VisitScript(chtl::ast::ScriptNode* node) override {
        std::string type = (node->GetType() == chtl::ast::ScriptNode::LOCAL) ? "local" : "global";
        PrintNode("Script (" + type + ")");
    }
    
    void VisitComment(chtl::ast::CommentNode* node) override {
        PrintNode("Comment: \"" + node->GetContent() + "\"");
    }
    
    void VisitTemplate(chtl::ast::TemplateNode* node) override {
        PrintNode("Template: " + node->GetName());
    }
    
    void VisitCustom(chtl::ast::CustomNode* node) override {
        PrintNode("Custom: " + node->GetName());
    }
    
    void VisitVar(chtl::ast::VarNode* node) override {
        PrintNode("Var: $" + node->GetName());
    }
    
    void VisitImport(chtl::ast::ImportNode* node) override {
        PrintNode("Import: " + node->GetPath());
    }
    
    void VisitNamespace(chtl::ast::NamespaceNode* node) override {
        PrintNode("Namespace: " + node->GetName());
        m_Indent++;
        for (const auto& child : node->GetChildren()) {
            child->Accept(this);
        }
        m_Indent--;
    }
    
    void VisitConfiguration(chtl::ast::ConfigurationNode* node) override {
        PrintNode("Configuration");
    }
    
    void VisitOrigin(chtl::ast::OriginNode* node) override {
        PrintNode("Origin");
    }
    
    void VisitInherit(chtl::ast::InheritNode* node) override {
        PrintNode("Inherit: " + node->GetBase());
    }
    
    void VisitDelete(chtl::ast::DeleteNode* node) override {
        PrintNode("Delete: " + node->GetTarget());
    }
    
    void VisitInsert(chtl::ast::InsertNode* node) override {
        PrintNode("Insert");
    }
    
    void VisitExcept(chtl::ast::ExceptNode* node) override {
        PrintNode("Except: " + node->GetConstraint());
    }
    
private:
    void PrintNode(const std::string& text) {
        for (int i = 0; i < m_Indent; i++) {
            std::cout << "  ";
        }
        std::cout << text << "\n";
    }
    
    int m_Indent;
};

int main(int argc, char* argv[]) {
    if (argc < 2) {
        PrintUsage(argv[0]);
        return 1;
    }
    
    // 解析命令行参数
    std::string inputFile;
    std::string outputFile;
    bool debug = false;
    bool prettyPrint = true;
    bool fullDocument = true;
    bool printAST = false;
    
    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];
        
        if (arg == "-h" || arg == "--help") {
            PrintUsage(argv[0]);
            return 0;
        } else if (arg == "-d" || arg == "--debug") {
            debug = true;
        } else if (arg == "-m" || arg == "--minify") {
            prettyPrint = false;
        } else if (arg == "-f" || arg == "--fragment") {
            fullDocument = false;
        } else if (arg == "--ast") {
            printAST = true;
        } else if (arg == "-o" || arg == "--output") {
            if (i + 1 < argc) {
                outputFile = argv[++i];
            } else {
                std::cerr << "Error: -o requires an argument\n";
                return 1;
            }
        } else if (arg[0] != '-') {
            if (inputFile.empty()) {
                inputFile = arg;
            } else if (outputFile.empty()) {
                outputFile = arg;
            }
        } else {
            std::cerr << "Error: Unknown option " << arg << "\n";
            return 1;
        }
    }
    
    if (inputFile.empty()) {
        std::cerr << "Error: No input file specified\n";
        return 1;
    }
    
    // 如果没有指定输出文件，使用默认名称
    if (outputFile.empty() && !printAST) {
        size_t lastDot = inputFile.find_last_of('.');
        if (lastDot != std::string::npos) {
            outputFile = inputFile.substr(0, lastDot) + ".html";
        } else {
            outputFile = inputFile + ".html";
        }
    }
    
    // 创建编译器
    chtl::CHTLCompilerV2 compiler;
    compiler.SetDebugMode(debug);
    compiler.SetPrettyPrint(prettyPrint);
    compiler.SetGenerateFullDocument(fullDocument);
    
    // 编译文件
    std::string html = compiler.CompileFile(inputFile);
    
    // 检查错误
    if (compiler.HasErrors()) {
        std::cerr << "Compilation failed with errors:\n";
        for (const auto& error : compiler.GetErrors()) {
            std::cerr << "  " << error << "\n";
        }
        return 1;
    }
    
    // 打印 AST
    if (printAST) {
        std::cout << "=== AST Structure ===\n";
        ASTPrinter printer;
        printer.Print(compiler.GetAST());
        std::cout << "\n";
    }
    
    // 输出结果
    if (!outputFile.empty()) {
        if (outputFile == "-") {
            std::cout << html;
        } else {
            std::ofstream output(outputFile);
            if (!output.is_open()) {
                std::cerr << "Error: Cannot write to file " << outputFile << "\n";
                return 1;
            }
            output << html;
            output.close();
            
            if (debug) {
                std::cout << "Successfully compiled " << inputFile << " to " << outputFile << "\n";
            }
        }
    }
    
    return 0;
}