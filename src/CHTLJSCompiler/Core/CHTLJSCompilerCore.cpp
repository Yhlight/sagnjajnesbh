...
CHTLJSCompilerCore::CHTLJSCompilerCore() : debug_mode_(false) {
    initialize();
}
...
bool CHTLJSCompilerCore::initialize() {
    try {
        ...
        if (debug_mode_) {
            std::cout << "CHTL JS编译器核心初始化成功" << std::endl;
        }
...
void CHTLJSCompilerCore::cleanup() {
    ...
    if (debug_mode_) {
        std::cout << "CHTL JS编译器核心已清理" << std::endl;
    }
}

void CHTLJSCompilerCore::reset() {
    clearErrors();
    cleanup();
    initialize();
}
...
std::string CHTLJSCompilerCore::compile(const std::string& source_code) {
    ...
    try {
        ...
        // 3. 虚对象优化处理
        virtual_object_manager_->optimizeAST(ast.get());
...
std::string CHTLJSCompilerCore::compileFile(const std::string& file_path) {
...
void CHTLJSCompilerCore::addError(const std::string& error) {
    ...
    if (debug_mode_) {
        std::cerr << "CHTL JS编译器错误: " << error << std::endl;
    }
}
...
void CHTLJSCompilerCore::addWarning(const std::string& warning) {
    ...
    if (debug_mode_) {
        std::cout << "CHTL JS编译器警告: " << warning << std::endl;
    }
}
